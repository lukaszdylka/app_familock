...
window.saveSess = function() {
  ...
  if (isVoucher && voucherId) {
    const v = S.vouchers.find(x => String(x.id) === String(voucherId));
      ...
      <div class="sess-edit-inner">
        ...
        <div class="field" id="se-voucher-field-${idx}" style="margin:0;${s.payment === 'voucher' ? '' : 'display:none'}">
          ...
          <select id="se-voucher-id-${idx}">
            ${(function() {
              const active = S.vouchers.filter(v => voucherStatus(v) === 'active' || String(v.id) === String(s.voucherId));
              return active.map(v =>
                `<option value="${v.id}" ${String(v.id) === String(s.voucherId) ? 'selected' : ''}>${esc(v.code)} · ${fmtPLN(v.value)}${v.buyer ? ` · ${esc(v.buyer)}` : ''}</option>`
              ).join('');
            })()}
          </select>
        </div>
...
window.updateSession = function(idx) {
  ...
  if (s.voucherId && (!isVoucher || $(`se-voucher-id-${idx}`)?.value !== s.voucherId)) {
    const oldV = S.vouchers.find(v => String(v.id) === String(s.voucherId));
  ...
  if (isVoucher) {
    ...
    const v = S.vouchers.find(x => String(x.id) === String(vId));
    s.payment     = 'voucher';
